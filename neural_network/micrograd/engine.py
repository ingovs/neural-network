...
class Value:
    ...
    # Backpropagation
    def backward(self):
        # Topological order of nodes in the graph (iterative to avoid recursion limit)
        topo = []
        visited = set()
        stack = [self]

        # Iterative topological sort to handle deep graphs
        while stack:
            node = stack[-1]
            if node in visited:
                stack.pop()
            ...
            all_children_visited = all(child in visited for child in node._previous)
