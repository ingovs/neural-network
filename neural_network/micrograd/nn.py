...
class Neuron:
    ...
    def __init__(self, num_inputs: int, activation: bool = True):
        # w = weights, b = bias
        # Xavier/He initialization for better gradient flow
        scale = (2.0 / num_inputs) ** 0.5 if activation else (1.0 / num_inputs) ** 0.5
    ...
    def __call__(self, x: List[Value]) -> Value:
        ...
        out = act.tanh() if self.activation else act  # Activation function (optional)
        return out
...
class Layer:
    ...
    def __init__(self, num_inputs: int, num_neurons: int, activation: bool = True):
        # num_neurons = number of neurons in the layer
        self.neurons = [
            Neuron(num_inputs, activation=activation) for _ in range(num_neurons)
        ]
...
class Conv2D:
    ...
    def __call__(self, x: List[List[List[Value]]]) -> List[List[List[Value]]]:
        ...
        assert in_channels == self.in_channels, (
            "Input channels must match layer's in_channels"
        )
        ...
        if self.padding > 0:
            padded_x = [
                [
                    [Value(0.0) for _ in range(in_width + 2 * self.padding)]
                    for _ in range(in_height + 2 * self.padding)
                ]
                for _ in range(in_channels)
            ]

            # # to better visualize the input before and after the padding
            # matrix_numbers = [
            #     [
            ...
            # after padding, copy original values into the center
        ...
        # Perform convolution
        output = [
            [[Value(0.0) for _ in range(out_width)] for _ in range(out_height)]
        ...
        for oc in range(self.out_channels):
            for oh in range(out_height):
                for ow in range(out_width):
                    ...
                    # Kernel convolution operation with input
                    conv_sum = Value(0.0)
...
class MLP:
    ...
    def __init__(self, num_inputs: int, layer_sizes: List[int]):
        ...
        self.layers = [
            Layer(num_inputs=sizes[i], num_neurons=sizes[i + 1])
...
class CNN:
    ...
    def gradient_descent_step(self, learning_rate: float = 0.01):
        """Update parameters using computed gradients. Call after loss.backward()."""
        for p in self.parameters():
            p.data -= learning_rate * p.gradient
            p.gradient = 0.0

    def zero_grad(self):
        """Reset all gradients to zero. Call this before each backward pass."""
        for p in self.parameters():
            p.gradient = 0.0

    def check_gradients(self):
        """Check if gradients have been computed (for debugging)."""
        params = self.parameters()
        if not params:
            return {"total_params": 0, "params_with_gradients": 0, "max_gradient": 0.0}

        params_with_grad = sum(1 for p in params if p.gradient != 0.0)
        max_grad = max(abs(p.gradient) for p in params)
